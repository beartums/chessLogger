...
chessLogger.controller('ChessLoggerCtrl', function($scope, $modal, $log, $timeout, $filter, mongoRestFactory,
	 ...
	 */
		...
		$scope.newClick = function() {
			...
			if ($scope.isGameDirty()) {
				...
				promise.then(function(ret) {
					...
					if  (ret=='Save') {
						var promise = $scope.saveGame(true);
						promise.then(
							function() {
								$scope.flashMessage('Promise resolved, game saved to db',true);
							},
							function() {
								$scope.flashMessage('Promise rejected, game saved only locally', false);
							});
					}
		...
		$scope.gameInit = function(game) {
			if (!game) {
				// get the gameinfo for the new game
